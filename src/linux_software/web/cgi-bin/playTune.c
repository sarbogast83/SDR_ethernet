...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    unsigned int bytes_transferred = 2048*4; // 4 bytes per call
	if (stop_time > start_time)
		time_spent = (stop_time-start_time)*8e-9; // period of 125 MHz
	else // clock rolled over
		time_spent = ((2^32)-stop_time+start_time)*8e-9; //2^32 counter size
	throughput = bytes_transferred/time_spent/1000000; // in MHz
    ...
    printf("<body>\n");
    printf("<h1>Hardware Performance Report</h1>\n");
    printf("<p>Data transferred: %d bytes</p>\n", bytes_transferred);
    printf("<p>Time taken: %f seconds</p>\n", time_spent);
    printf("<h2>Measured Transfer throughput = %f Mbytes/sec</h2>\n", throughput);
    printf("</body>\n");
    printf("</html>\n");
}
